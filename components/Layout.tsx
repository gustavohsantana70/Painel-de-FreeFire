...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const navItems = [
    ...
    { label: 'Arquivos', icon: <FolderOpen size={24} />, path: '/files' }, // Nova aba
    { label: 'Configs', icon: <Settings size={24} />, path: '/settings' },
  ];

  const isActive = (path: string) => location.pathname === path;

  // Renderização do Ícone Flutuante (Quando minimizado)
  if (isMinimized) {
    return (
      <div className="h-screen w-full relative pointer-events-none">
        {/* O ícone flutuante deve ser clicável */}
        <button 
          onClick={() => setIsMinimized(false)}
          className="pointer-events-auto fixed top-1/4 left-4 md:left-10 w-16 h-16 rounded-full bg-black border-2 border-neon shadow-[0_0_20px_#00ff00] flex items-center justify-center animate-bounce z-50 group hover:scale-110 transition-transform"
        >
           <Skull size={32} className="text-neon group-hover:animate-pulse" />
           <span className="absolute -bottom-6 bg-black text-neon text-[10px] font-bold px-2 py-0.5 rounded border border-neon/50">
             ABRIR
           </span>
        </button>
      </div>
  ...
  return (
    <div className="h-screen w-full flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-[850px] h-[600px] md:h-[550px] bg-black border-2 border-neon rounded-xl flex flex-col shadow-[0_0_50px_rgba(0,255,0,0.2)] overflow-hidden relative animate-in fade-in zoom-in duration-300">
        
        {/* Panel Header */}
        <header className="h-14 bg-neon flex items-center justify-between px-4 shrink-0 select-none">
          ...
          <div className="flex items-center gap-3 text-black">
            <button className="hover:bg-black/10 p-1 rounded" title="Restaurar"><Maximize2 size={18} /></button>
            <button 
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* Sidebar */}
          <aside className="w-20 bg-[#0a0a0a] flex flex-col items-center py-6 gap-4 border-r border-[#222]">
            {navItems.map((item) => (
              <button
                key={item.path}
